...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const proAside = document.getElementById('ProductInfoAside');
    const threshold = 200;
    ...
    window.addEventListener('scroll', () => {
        ...
        if (curScroll > threshold) {
            ...
            if (proAside) proAside.classList.add('sticky');

            if (curScroll > prevScroll) {
                header.classList.add('hide');
                if (proAside) proAside.classList.add('hide');
            } else {
                header.classList.remove('hide');
                if (proAside) proAside.classList.remove('hide');
            }
        } else {
            header.classList.remove('sticky', 'hide');
            if (proAside) proAside.classList.remove('sticky', 'hide');
        }
    ...
    document.querySelectorAll('.cNacBtn').forEach(button => {
        button.addEventListener('click', () => {
            document.querySelectorAll('.cNacBtn').forEach(btn => btn.classList.toggle(
                'open'));
    ...
    function highlightCurrentPage() {
        // Get the current page name (file name)
        const currentPage = window.location.pathname.split("/").pop();

        // Select all anchor tags in the header
        const links = document.querySelectorAll("header a");

        // Loop through all the links and check if the href matches the current page name
        links.forEach(function (link) {
            const href = link.getAttribute("href");

            // Check if the link's href is the same as the current page name
            if (href === currentPage) {
